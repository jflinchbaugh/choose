...
The `home-routes` handler in the `choose.routes.home` namespace
...
The `home-page` function will in turn call the `choose.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/choose/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `choose.handler` namespace:
...
Request middleware functions are located under the `choose.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `choose.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `choose.db.core` in some other namespace.
